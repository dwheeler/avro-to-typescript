...
class RecordConverter extends BaseConverter_1.BaseConverter {
    ...
    convertType(type) {
        if (typeof type === "string") {
            const converter = new PrimitiveConverter_1.PrimitiveConverter(undefined, this.primitiveTypesMap);
            return converter.convert(type);
        }
        if (TypeHelper_1.TypeHelper.isLogicalType(type)) {
            const converter = new LogicalTypeConverter_1.LogicalTypeConverter(this.logicalTypesMap, this.primitiveTypesMap);
            return converter.convert(type);
        }
    ...
    getField(name, type) {
        return `${name}${TypeHelper_1.TypeHelper.isOptional(type) ? "?" : ""}: ${this.convertType(type)}`;
    }
}
