...
export class Compiler extends BaseCompiler {
    ...
    public constructor(
      ...
      public primitiveTypes?: { [key: string]: string },
    ) {
        super();
    ...
    public async compile(data: any): Promise<CompilerOutput> {
        const classConverter = new ClassConverter(this.logicalTypes, this.primitiveTypes);
    ...
    protected saveBaseAvroRecord() {
        ...
        if (!fs.existsSync(avroRecordPath)) {
            fs.writeFileSync(
                avroRecordPath,
                "export { BaseAvroRecord } from \"avro-to-typescript\";\n",
            );
        }
    }
}
